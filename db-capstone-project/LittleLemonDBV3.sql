...
INSERT INTO `bookings` VALUES (1,1,12,'2022-10-10','19:00:00',1),(2,2,13,'2022-11-12','19:00:00',1),(3,3,19,'2022-10-12','15:00:00',3),(4,4,15,'2022-10-13','17:30:00',4),(5,5,5,'2022-11-15','18:30:00',2),(6,6,8,'2022-10-18','20:00:00',5),(87,1,6,'2024-02-28','21:30:00',4),(102,2,9,'2024-03-01','19:15:00',3);
...
INSERT INTO `orders` VALUES (3,15,2,3,37,3,3),(4,5,3,4,40,5,4),(5,8,1,5,43,1,2);
...
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `AddBooking`(IN Customer_ID INT, IN Table_No INT, IN Booking_Date DATE,IN Booking_Slot TIME, IN Employee_ID INT)
BEGIN
    DECLARE RecordCount INT;
    START TRANSACTION;

    -- Add a new booking record
    INSERT INTO bookings (CustomerID,TableNo,BookingDate,BookingSlot,EmployeeID) 
    VALUES (Customer_ID,Table_No,Booking_Date, Booking_Slot,Employee_ID);
    ...
    IF RecordCount > 1 THEN
        ROLLBACK;
        SELECT CONCAT('Booking for Table ', Table_No,' for Customer Number ',Customer_ID,' on ', Booking_Date,' and Booking slot ' ,Customer_ID,' declined. Table already booked.') AS 'Confirmation';
    ELSE
        -- If the table is available, commit the transaction
        COMMIT;
        SELECT CONCAT('Booking for Table ', Table_No,' for Customer Number ',Customer_ID,' on ', Booking_Date,' and Booking slot ' ,Customer_ID, ' confirmed.') AS 'Confirmation';
    END IF;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `AddValidBooking`(IN Booking_Date DATE, IN Table_No INT)
BEGIN
    DECLARE RecordCount INT;
    START TRANSACTION;
    ...
    IF RecordCount > 1 THEN
        ROLLBACK;
        SELECT CONCAT('Booking for Table ', Table_No, ' on ', Booking_Date, ' declined. Table already booked.') AS 'Booking Status';
    ELSE
        -- If the table is available, commit the transaction
        COMMIT;
        SELECT CONCAT('Booking for Table ', Table_No, ' on ', Booking_Date, ' confirmed.') AS 'Booking Status';
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
    ...
    IF ROW_COUNT() > 0 THEN
        ...
        COMMIT;
        SELECT CONCAT('Booking with ID ', Booking_ID, ' canceled successfully. Rows affected: ', ROW_COUNT()) AS 'Cancellation Status';
    ELSE
        -- If no rows were affected, rollback the transaction
        ROLLBACK;
        SELECT CONCAT('No booking found with ID ', Booking_ID, '. Cancellation failed. Rows affected: ', ROW_COUNT()) AS 'Cancellation Status';
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `CheckBooking`(IN Booking_Date DATE, IN Table_No INT)
BEGIN
    DECLARE TableStatus INT;

    -- Check if the table is already booked on the given date
    SELECT BookingID INTO TableStatus
    FROM bookings
    WHERE BookingDate = Booking_Date AND TableNo = Table_No;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `UpdateBooking`(IN Booking_ID INT,IN New_Booking_Date DATE, IN New_Table_No INT , IN New_Booking_Slot TIME)
BEGIN
 DECLARE RecordCount INT;
    START TRANSACTION;
    ...
    -- Add a new booking record
    UPDATE bookings
    SET BookingDate = New_Booking_Date , TableNo = New_Table_No , BookingSlot= New_Booking_Slot
    WHERE BookingID = Booking_ID;
	...
	IF ROW_COUNT() > 0 THEN
		COMMIT;
        SELECT CONCAT('Booking with ID ', Booking_ID,
        ' updated to ', New_Booking_Date, ' for Table ', New_Table_No, ' at Slot ', New_Booking_Slot) AS 'Update Status';
	ELSE
        ROLLBACK;
        SELECT CONCAT('No booking found with ID ', Booking_ID) AS 'Update Status';
...
-- Dump completed on 2024-02-28 18:32:10
